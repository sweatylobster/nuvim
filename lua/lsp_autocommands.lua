...
local organize_imports = function(client, bufnr)
  ...
  local handler = function(err, result, context, config)
    if err then
      return -- no problem if you can't organize imports :)
    end
  ...
  local params = vim.lsp.util.make_range_params(win, "utf-16")
  params.context = { any = "any", only = { "source.organizeImports" } }
    ...
    callback = function()
      ...
      local format = function(client, bufnr)
        if client.server_capabilities.documentFormattingProvider then
          vim.lsp.buf.format({ bufnr = bufnr, timeout_ms = 4000, id = client.id })
        end
      end
